...
final class MeetSetupViewReactor: Reactor, LifeCycleLoggable {
    ...
    func reduce(state: State, mutation: Mutation) -> State {
        ...
        case let .updateMeet(meet):
            print(#function, #line, "Path : # 변경 업데이트 ")
            newState.meet = meet
        case let .checkHost(isHost):
            print(#function, #line, "Path : # 변경 업데이트 \(isHost)")
...
extension MeetSetupViewReactor {
    private func handleFlowAction(_ action: Action.Flow) -> Observable<Mutation> {
        ...
        case .transferMeet:
            guard let meet = currentState.meet else { break }
            coordinator?.pushTransferMeetView(meet: meet)
        case .pop:
            coordinator?.pop()
        case .endFlow:
            coordinator?.endFlow()
