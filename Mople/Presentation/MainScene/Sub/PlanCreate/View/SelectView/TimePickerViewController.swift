...
final class TimePickerViewController: BaseViewController, View {
    ...
    // MARK: - UI Components
    private let pickerView = CustomPickerView(title: TextStyle.CreatePlan.Picker.time)
    ...
    private func outputBind(_ reactor: Reactor) {
        reactor.pulse(\.$selectedTime)
            .compactMap({ [weak self] in
                return self?.configurePeriodSettings(on: $0)
            })
