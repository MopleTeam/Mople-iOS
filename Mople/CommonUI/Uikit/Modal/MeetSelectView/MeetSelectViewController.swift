...
final class MeetSelectViewController: BaseViewController {
    ...
    private var meetList: [MeetSummary]
...
// MARK: - Paging
extension MeetSelectViewController: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        guard hasNextPage,
              scrollView.isBottom(threshold: 50) else { return }
        onNextPage?()
    }
}
...
extension MeetSelectViewController {
    func updateMeetList(_ meets: [MeetSummary]) {
        self.meetList = meets
        self.tableView.reloadData()
    }
    
    func updateHasNextPage(_ hasNext: Bool) {
