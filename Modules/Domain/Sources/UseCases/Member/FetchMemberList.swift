...
public protocol FetchMemberList {
    func execute(type: MemberListType, cursor: String?) async throws -> Page<MemberInfo>
}

public final class FetchMemberUseCase: FetchMemberList {

    private let memberListRepo: MemberRepo

    public init(memberListRepo: MemberRepo) {
        self.memberListRepo = memberListRepo
    }

    public func execute(type: MemberListType, cursor: String?) async throws -> Page<MemberInfo> {
        return try await memberListRepo.execute(type: type, nextCursor: cursor)
    }
}

public final class MockFetchMemberUseCase: FetchMemberList {
    public init() {}
