...
public protocol FetchMeetReviewList {
    func execute(meetId: Int,
                 cursor: String?) async throws -> Page<Review>
}

public final class FetchMeetReviewListUseCase: FetchMeetReviewList {

    private let repo: ReviewRepo
    private let session: UserSessionProvider

    public init(repo: ReviewRepo, session: UserSessionProvider) {
        self.repo = repo
...
// MARK: - Mock UseCase
#if DEV
public final class MockFetchMeetReviewListUseCase: FetchMeetReviewList {
    public init() {}

    public func execute(meetId: Int, cursor: String?) async throws -> Page<Review> {
